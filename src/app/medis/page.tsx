...
export default function MedisDashboard() {
  ...
  useEffect(() => {
    ...
    const sosChannel = supabase.channel('medis_sos').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'laporan_darurat' }, (payload) => {
        setSosList(prev => [payload.new, ...prev].slice(0, 10));
      }).subscribe();
    return () => { clearInterval(timer); supabase.removeChannel(sosChannel); };
  }, []);

  return (
    // DIUBAH: overflow-y-auto diaktifkan agar bisa di-scroll di HP
    <main className="min-h-[100dvh] bg-[#040605] text-white p-4 lg:p-6 font-mono flex flex-col gap-6 relative overflow-x-hidden overflow-y-auto pb-10">
      ...
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-emerald-900/50 pb-4 gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-gray-400">Dr. Naila | Otorisasi Aktif</p>
          </div>
        </div>
      ...
      {/* DIUBAH: flex-col pada layar kecil, lg:flex-row untuk layar besar */}
      <div className="flex flex-col lg:flex-row gap-6 h-full flex-1">
        ...
        <section className="w-full lg:w-1/2 flex flex-col gap-4">
          ...
          <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
            {sosList.map((sos, idx) => (
              <div key={idx} className="bg-red-950/20 border border-red-900/30 rounded-xl p-4 flex flex-col gap-3 relative">
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-500"></div>
                <div><h3 className="font-bold text-red-400 text-sm mb-1 uppercase">{sos.nama_korban || 'KORBAN TERDETEKSI'}</h3><p className="text-[10px] text-gray-400">Koordinat: {sos.latitude}, {sos.longitude}</p></div>
                <button onClick={() => setSelectedKorban(sos)} className="w-full py-2 bg-red-600/20 text-red-400 border border-red-500/30 rounded-lg text-[10px] font-bold">TINDAK LANJUTI</button>
              </div>
            ))}
          </div>
        </section>
      ...
      {selectedKorban && (
        <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-4">
          <div className="bg-[#0a0f0d] border-2 border-emerald-500/50 rounded-2xl w-full max-w-md p-5 flex flex-col">
            ...
            <div className="bg-red-950/20 border border-red-500/30 p-4 rounded-xl mb-6"><p className="text-[9px] text-red-400/80 mb-2">KELUHAN MEDIS</p><p className="text-sm font-bold">{selectedKorban.kondisi_medis}</p></div>
            <div className="flex gap-2">
              <button onClick={() => setSelectedKorban(null)} className="flex-1 py-3 bg-white/5 rounded-xl text-xs font-bold text-gray-400">TUTUP</button>
              <a href={`https://www.google.com/maps/dir/?api=1&destination=${selectedKorban.latitude},${selectedKorban.longitude}`} target="_blank" className="flex-1 py-3 bg-emerald-600 text-white rounded-xl text-xs font-bold text-center flex items-center justify-center gap-2">RUTE MAPS</a>
            </div>
          </div>
        </div>
      )}
