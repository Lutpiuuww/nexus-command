  ...
  const handleEksekusi = async () => {
    ...
    try {
      ...
      if (selectedLaporan.nama_korban.includes("SIMULASI")) {
        ...
        if (insErr) { alert("GAGAL MENYIMPAN KE DATABASE! Pastikan tabel 'penugasan_relawan' sudah ada."); throw insErr; }
      ...
      <div className="w-full max-w-6xl mx-auto px-6 mt-8 relative z-10">
        ...
        {globalStatus === 'KRITIS' && disasterInfo && (
          <div className="mb-6 bg-red-950/20 border-2 border-red-500 rounded-xl p-5 shadow-[0_0_30px_rgba(239,68,68,0.2)] animate-in slide-in-from-top-2 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              ...
              <p className="text-xs text-red-300 mt-1">Sistem Pusat mendeteksi ancaman di koordinat <span className="font-bold text-white">{disasterInfo.lat}, {disasterInfo.lon}</span> dengan radius bahaya {disasterInfo.radius} KM.</p>
            </div>
            <a 
          ...
          <div className="col-span-2 bg-black/40 border border-white/10 p-4 rounded-xl backdrop-blur-sm flex justify-between items-center">
            <div>
              ...
              <p className={`text-xs font-bold flex items-center gap-2 ${globalStatus === 'KRITIS' ? 'text-red-500' : 'text-emerald-400'}`}>
                ...
                {globalStatus === 'KRITIS' ? `CRITICAL ALERT: ${disasterInfo?.jenis || 'ACTIVE'}` : 'ONLINE & SECURE'}
              </p>
            </div>
        ...
        {laporanList.length === 0 ? (
          <div className={`w-full h-80 bg-black/30 border rounded-2xl flex flex-col items-center justify-center relative overflow-hidden backdrop-blur-sm transition-colors duration-500 ${globalStatus === 'KRITIS' ? 'border-red-500/30 shadow-[inset_0_0_50px_rgba(239,68,68,0.1)]' : 'border-white/5'}`}>
            ...
            {/* PERBAIKAN: Teks Spesifik Ancaman */}
            <div className="mt-8 text-center z-10">
              <p className={`text-sm font-black tracking-[0.3em] uppercase ${globalStatus === 'KRITIS' ? 'text-red-500 animate-pulse' : 'text-cyan-400'}`}>
                {globalStatus === 'KRITIS' ? `ANCAMAN ${disasterInfo?.jenis || 'DARURAT'} TERDETEKSI` : 'ZONA AMAN TERKENDALI'}
              </p>
              <p className={`text-[10px] tracking-widest mt-2 uppercase ${globalStatus === 'KRITIS' ? 'text-red-400' : 'text-gray-500'}`}>
                {globalStatus === 'KRITIS' ? `Status Siaga Penuh. Siapkan armada evakuasi!` : 'Sistem radar memindai anomali...'}
              </p>
            </div>
          </div>
        ) : (
          ...
          <div className="bg-[#0a0a14] border border-cyan-500/30 rounded-2xl w-full max-w-lg overflow-hidden shadow-[0_0_50px_rgba(34,211,238,0.15)] animate-in zoom-in duration-200">
            ...
            <div className="p-6 flex flex-col gap-6">
              <a href={`https://www.google.com/maps/dir/?api=1&destination=${selectedLaporan.latitude},${selectedLaporan.longitude}`} target="_blank" rel="noopener noreferrer" className="w-full py-3 bg-blue-900/20 hover:bg-blue-600/40 border border-blue-500/50 text-blue-400 rounded-xl text-[10px] font-bold tracking-widest flex items-center justify-center gap-2 transition-all shadow-[0_0_15px_rgba(59,130,246,0.15)]">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg> BUKA RUTE KORBAN (GOOGLE MAPS)
              </a>
              <div>
