  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data: tugasData } = await supabase
        ...
        .select('*')
        .order('id_tugas', { ascending: false })
        .limit(10);
      
      const { data: laporanData } = await supabase
        .from('laporan_darurat')
        .select('*')
      ...
      // ==========================================
      // LOGIKA ANTI-TABRAKAN DATA (TRACKER)
      // ==========================================
      if (tugasData && laporanData) {
        const namaTercatat = new Set(); // Buku catatan kita

        const mergedList = tugasData.map(tugas => {
          // 1. Bersihkan nama dari spasi nyasar & ubah ke huruf besar semua agar cocok 100%
          const namaPelaporAsli = (tugas.nama_pelapor || '').trim().toUpperCase();

          // 2. Cari data laporan di Supabase dengan nama yang sudah dibersihkan
          const laporan = laporanData.find(l => (l.nama_korban || '').trim().toUpperCase() === namaPelaporAsli);

          // 3. Apakah ini kartu pertama (terbaru) untuk nama ini?
          const isTugasTerbaru = !namaTercatat.has(namaPelaporAsli);

          // 4. Masukkan nama ke buku catatan agar kartu lama dengan nama sama tidak terbaca lagi
          namaTercatat.add(namaPelaporAsli);

          return {
            ...tugas,
            // JIKA terbaru, ambil status aslinya. JIKA kartu lama, PAKSA KUNCI jadi 'selesai'
            status_korban: isTugasTerbaru ? (laporan?.status || 'darurat') : 'selesai',
            lat: laporan?.latitude,
            lon: laporan?.longitude
          };
        });
        setTugasList(mergedList);
      }
    };
    ...
    const tugasChannel = supabase.channel('lapangan_tugas')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'penugasan_relawan' }, () => {
        fetchData();
      }).subscribe();

    const sosChannel = supabase.channel('lapangan_sos')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'laporan_darurat' }, () => {
        fetchData();
      }).subscribe();
      ...
      <div className="flex-1 flex flex-col gap-4">
         <h2 className="text-sm font-bold tracking-widest text-gray-300 flex items-center gap-2">
          ...
          <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-4 overflow-y-auto flex flex-col gap-3">
             ...
             {tugasList.map((tugas) => {
               const isAman = tugas.status_korban === 'selesai';

               return (
                 <div key={tugas.id_tugas} className={`border rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center transition-all group duration-500
                   ${isAman ? 'bg-emerald-950/10 border-emerald-900/30' : 'bg-black/40 border-white/10 hover:border-red-500/50'}`}
                 >
                   <div className="mb-3 sm:mb-0">
                     <h3 className={`font-bold text-sm mb-1 uppercase tracking-widest ${isAman ? 'text-emerald-500/50' : 'text-red-400'}`}>
                       PELAPOR: {tugas.nama_pelapor || 'Warga (Sinyal Darurat GPS)'}
                     </h3>
                   ...
                   <div className="w-full sm:w-auto">
                     {isAman ? (
                        <div className="w-full py-2 px-4 bg-emerald-900/20 text-emerald-400 border border-emerald-500/30 rounded-lg text-[10px] font-bold tracking-widest uppercase flex items-center justify-center gap-2 animate-pulse shadow-[0_0_15px_rgba(16,185,129,0.1)]">
                          <span className="text-xs">✅</span> BERHASIL DIEVAKUASI
                        </div>
                     ) : (
                        <button 
                          onClick={() => setSelectedTugas(tugas)}
                          className="w-full px-4 py-2 bg-red-900/20 hover:bg-red-600 text-red-400 hover:text-white border border-red-500/30 hover:border-red-400 rounded-lg text-[10px] font-bold tracking-widest uppercase transition-all cursor-pointer shadow-[0_0_15px_rgba(239,68,68,0.1)] group-hover:shadow-[0_0_20px_rgba(239,68,68,0.4)]"
                        >
                          DETAIL / RUTE MAPS
                        </button>
                     )}
          ...
          <div className="bg-[#0a0a14] border-2 border-cyan-500/50 rounded-2xl w-full max-w-md p-6 shadow-[0_0_40px_rgba(34,211,238,0.2)] flex flex-col">
            ...
            <div className="flex flex-col gap-3 mb-8">
              ...
              <div className="bg-red-950/20 border border-red-500/30 p-4 rounded-xl relative overflow-hidden">
                ...
                {selectedTugas.lat && selectedTugas.lon ? (
                  <p className="text-xs font-bold text-red-400 font-sans">KOORDINAT: {selectedTugas.lat}, {selectedTugas.lon}</p>
                ) : (
            ...
            <div className="flex gap-3 mt-auto">
              ...
              {selectedTugas.lat && selectedTugas.lon ? (
                <a
                  href={`https://www.google.com/maps/dir/?api=1&destination=${selectedTugas.lat},${selectedTugas.lon}`}
                  target="_blank"
