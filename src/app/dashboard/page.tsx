    ...
    const laporanChannel = supabase.channel('listen_laporan')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'laporan_darurat' }, (payload) => {
        ...
        if (payload.eventType === 'INSERT') {
          ...
          // 3. MENYAMBUNGKAN URL FOTO DARI DATABASE KE DALAM LOG (pastikan kolom photo_url sudah ada di Supabase)
    ...
    <main className="h-screen w-full bg-[#06060c] text-white p-4 lg:p-6 font-sans flex flex-col lg:flex-row items-stretch gap-6 overflow-hidden relative">
      ...
      <section className="w-full lg:w-1/4 flex flex-col gap-3 z-10">
        <div className="flex-1 min-h-300px lg:min-h-0 border border-white/10 bg-black/40 rounded-2xl p-4 lg:p-5 flex flex-col shadow-lg">
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-3 pr-2">
            {logs.map((log) => (
              <div key={log.id} className="flex flex-col gap-1 border-b border-white/5 pb-3">
                <span className="text-[10px] text-gray-500">{log.time}</span>
                <p className={`text-xs ${log.type === 'SOS' ? 'text-red-400' : log.type === 'SAFE' ? 'text-emerald-400' : 'text-cyan-400'}`}>{log.msg}</p>
                
                {/* 4. TOMBOL VERIFIKASI (Akan muncul hanya jika photo_url tidak kosong) */}
                {log.photo_url && (
                  <button 
                    onClick={() => setEvidenceModal({ isOpen: true, url: log.photo_url!, senderName: log.nama || 'Warga' })}
                    className="mt-2 flex items-center justify-center gap-2 text-[10px] tracking-wider text-cyan-400 hover:text-white transition-colors border border-cyan-900 bg-cyan-950/40 px-3 py-1.5 rounded cursor-pointer w-max"
                  >
                    👁️ VERIFIKASI BUKTI
                  </button>
                )}
      ...
      {evidenceModal.isOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/85 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl shadow-[0_0_40px_rgba(255,0,0,0.15)] max-w-2xl w-full overflow-hidden flex flex-col">
            
            {/* Header Modal */}
            <div className="px-6 py-4 border-b border-[#2a2a2a] flex justify-between items-center bg-[#111]">
              <div>
                <h3 className="text-red-500 font-bold tracking-widest text-sm uppercase">TARGET ACQUIRED</h3>
                <p className="text-gray-400 text-xs mt-1">Bukti visual dari: <span className="text-white font-bold">{evidenceModal.senderName}</span></p>
              </div>
              <button 
                onClick={() => setEvidenceModal({ isOpen: false, url: '', senderName: '' })}
            ...
            <div className="relative p-6 flex justify-center items-center bg-black min-h-[300px]">
              ...
              <div className="absolute top-4 left-4 w-6 h-6 border-t-2 border-l-2 border-red-900/50"></div>
              <div className="absolute top-4 right-4 w-6 h-6 border-t-2 border-r-2 border-red-900/50"></div>
              <div className="absolute bottom-4 left-4 w-6 h-6 border-b-2 border-l-2 border-red-900/50"></div>
              <div className="absolute bottom-4 right-4 w-6 h-6 border-b-2 border-r-2 border-red-900/50"></div>
            ...
            <div className="p-6 bg-[#0a0a0a] border-t border-[#2a2a2a] flex justify-end gap-4">
              <button 
                onClick={() => setEvidenceModal({ isOpen: false, url: '', senderName: '' })}
                className="px-6 py-2 rounded text-sm font-medium text-gray-400 hover:text-white border border-gray-800 hover:bg-gray-800 transition-all cursor-pointer"
              >
                ABAIKAN (PRANK)
              </button>
              <button 
                onClick={() => {
                  setEvidenceModal({ isOpen: false, url: '', senderName: '' });
                  handleDispatch(); // Langsung eksekusi pasukan lapangan dari modal
                }}
                className="px-6 py-2 rounded text-sm font-bold tracking-wider bg-red-900/80 hover:bg-red-600 text-white shadow-[0_0_15px_rgba(255,0,0,0.4)] border border-red-500/50 transition-all cursor-pointer"
              >
                CONFIRM & DISPATCH
              </button>
            </div>
